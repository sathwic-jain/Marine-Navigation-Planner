...
public class A1mainforalgorithmdata {
	...
	public static void main(String[] args) {
		...
		if (args[0].equals("all")) {
			Conf confs[] = Conf.values();
			String[] algorithms = new String[] { "BFS", "DFS", "BestF", "AStar", "BidirectionalAStar",
					"tripleAgentBestFS" };
			ArrayList<AlgorithmData> stats = new ArrayList<AlgorithmData>();
			for (int j = 0; j < algorithms.length; j++) {
				// stats.clear();
				System.out.println(algorithms[j]);
				for (int i = 0; i < confs.length; i++) {

					stats.add(getSearchStats(algorithms[j], confs[i].getMap(), confs[i].getS(), confs[i].getG()));
				}

			}
			for (int i = 0; i < algorithms.length; i++) {
				System.out.println(algorithms[i] + " --->");
				System.out.println(" nodesTraversed  cost  timeTaken(ms) ");
				for (int j = confs.length * i; j < confs.length * (i + 1); j++) {
					System.out.print("|       ");
					System.out.print(stats.get(j).getNodesTraversed());
					System.out.print("|         ");
					System.out.print("|  ");
					System.out.print(stats.get(j).getCost());
					System.out.print("  |");
					System.out.print("      ");
					System.out.print(stats.get(j).getTimeTaken());
					System.out.print("     |");
					System.out.println();
				}
				System.out.println();
		...
		// } else {
			...
			// runSearch("AStar",conf.getMap(),conf.getS(),conf.getG());

		}
	//} // bracket close for if else condition for data retrievel section of code

	//In the following function the commented part of the code was used to retrieve the data necessary for the analysis.
	
	private static AlgorithmData getSearchStats(String algo, Map map, Coord start, Coord goal) {
		...
		switch (algo) {
			...
			case "BidirectionalAStar":
				return (BidirectionalAStar(map, start, goal));
				// BidirectionalAStar(map, start, goal);
      			// break;
			case "tripleAgentBestFS":
				return (tripleAgentBestFS(map, start, goal));
	...
	/**
	 * Commands used to get statistics check for algorithm follows.
	 * These are required to be uncommented inorder to do the same.
	 * Additionally the main static declaration of the function has to
	 * be commented inorder to avoid multiple function declaration.
	 */
	public static AlgorithmData BFS(Map map, Coord start, Coord goal) {
		// Time variable for stats funtion to check each algorithm for each
		// configuration.
		long startTime = System.currentTimeMillis();
		AlgorithmData algorithmStats = new AlgorithmData();
		/**
		 * Code for statsCheck ends here
		 */
		...
		/**
		 * This part is commented as it is a part
		 * of statscheck for the algorithm.
		 */
		long endTime = System.currentTimeMillis();
		algorithmStats.setTimeTaken(endTime - startTime);
		algorithmStats.setNodesTraversed(breadthFS.getNodesTraversed());
		algorithmStats.setCost(breadthFS.getFinalCost());
		return algorithmStats;
		/**
		 * Code that gathers data for statscheck ends here.
		 */
	}
	 ...
	 * be commented inorder to avoid multiple function declaration.
	 */
	public static AlgorithmData DFS(Map map, Coord start, Coord goal) {
		// Time variable for stats funtion to check each algorithm for each
		// configuration.
		long startTime = System.currentTimeMillis();
		AlgorithmData algorithmStats = new AlgorithmData();
		...
		long endTime = System.currentTimeMillis();
		algorithmStats.setTimeTaken(endTime - startTime);
		algorithmStats.setNodesTraversed(depthFS.getNodesTraversed());
		algorithmStats.setCost(depthFS.getFinalCost());
		return algorithmStats;
		/**
		 * Code that gathers data for statscheck ends here.
		 */
	 ...
	 * be commented inorder to avoid multiple function declaration.
	 */
	public static AlgorithmData BestFS(Map map, Coord start, Coord goal) {
		// Time variable for stats funtion to check each algorithm for each
		// configuration.
		...
		long endTime = System.currentTimeMillis();
		algorithmStats.setTimeTaken(endTime - startTime);
		algorithmStats.setNodesTraversed(bestFirst.getNodesTraversed());
		algorithmStats.setCost(bestFirst.getFinalCost());
		return algorithmStats;
		/**
		 * Code that gathers data for statscheck ends here.
		 */
	 ...
	 * be commented inorder to avoid multiple function declaration.
	 */
	public static AlgorithmData AStar(Map map, Coord start, Coord goal) {
		// Time variable for stats funtion to check each algorithm for each
		// configuration.
		long startTime = System.currentTimeMillis();
		AlgorithmData algorithmStats = new AlgorithmData();
		...
		long endTime = System.currentTimeMillis();
		algorithmStats.setTimeTaken(endTime - startTime);
		algorithmStats.setNodesTraversed(aStarSearch.getNodesTraversed());
		algorithmStats.setCost(aStarSearch.getFinalCost());
		return algorithmStats;
		/**
		 * Code that gathers data for statscheck ends here.
		 */
	 ...
	 * be commented inorder to avoid multiple function declaration.
	 */
	public static AlgorithmData BidirectionalAStar(Map map, Coord start, Coord goal) {
		// Time variable for stats funtion to check each algorithm for each
		// configuration.
		long startTime = System.currentTimeMillis();
		AlgorithmData algorithmStats = new AlgorithmData();
		...
		outer:while (!frontier.isEmpty() && !reverseFrontier.isEmpty()) {
			...
			*/
			...
			biAStar.printFrontierAStar(reverseFrontier);
			/**
			 * Implementing a failsafe case for missing the node from either direction.
			 * Case : if the currentNode/reverseCurrentNode is equal to the node that is to
			 * be popped/expanded from the alternate frontier.
			 * This case takes care of a condition where the nodes may miss each other since
			 * we are only comparing the popped nodes.
			 * This case further prevents unnecessary cost for the nodes as it decreases the
			 * node required
			 * to get to the goal.
			 */
			// if (!frontier.isEmpty() && !reverseFrontier.isEmpty()) {
			// 	if (biAStar.goalCheck(currentNode, reverseFrontier.get(0).getCoord())) {
			// 		System.out.println("here");
			// 		currentReverseNode = reverseFrontier.remove(0);
			// 		try {
			// 			NodeInformed dummyNode = (NodeInformed) currentNode.clone();
			// 			NodeInformed dummyNodeReversed = (NodeInformed) currentReverseNode.clone();
			// 			// printing both the paths
			// 			biAStar.printPath(dummyNode);
			// 			biAStar.printPath(dummyNodeReversed);
			// 		} catch (Exception e) {
			// 			// printing clone error exception
			...
			// 		// breaks of the loop if the goal is reached.
			// 		break;
			// 	} else if (biAStar.goalCheck(currentReverseNode, frontier.get(0).getCoord())) {
		...
		long endTime = System.currentTimeMillis();
		algorithmStats.setTimeTaken(endTime - startTime);
		algorithmStats.setNodesTraversed(biAStar.getNodesTraversed());
		algorithmStats.setCost(biAStar.getFinalCost());
		return algorithmStats;
		/**
		 * Code that gathers data for statscheck ends here.
		 */
	 ...
	 * be commented inorder to avoid multiple function declaration.
	 */
	public static AlgorithmData tripleAgentBestFS(Map map, Coord start, Coord goal) {
		// Time variable for stats funtion to check each algorithm for each
		// configuration.
		long startTime = System.currentTimeMillis();
		AlgorithmData algorithmStats = new AlgorithmData();
		...
		if (!(startNode == null) && !(adjacentAgentOne == null)) {
			...
			while (!frontier.isEmpty()) {
				...
				bestFirstTriple.printFrontierBestF(frontier);
				// bestFirstTriple.printFrontierBestF(PathNodeOne);
				// bestFirstTriple.printFrontierBestF(PathNodeTwo);
				/**
		...
		long endTime = System.currentTimeMillis();
		algorithmStats.setTimeTaken(endTime - startTime);
		algorithmStats.setNodesTraversed(bestFirstTriple.getNodesTraversed());
		algorithmStats.setCost(bestFirstTriple.getFinalCost());
		return algorithmStats;
		/**
		 * Code that gathers data for statscheck ends here.
		 */
